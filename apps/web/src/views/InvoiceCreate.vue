...
async function submit() {
  ...
  if (!result.success) {
    error.value = JSON.stringify(flattenError(result.error).fieldErrors, null, 2)
    return
  }
  error.value = ''
...
<template>
  <form @submit.prevent="submit" class="max-w-2xl mx-auto p-6 space-y-6">
    <div class="space-y-4">
      ...
      <div class="grid grid-cols-2 gap-4">
        <input
          ...
          maxlength="3"
          placeholder="USD"
          class="border p-3 rounded uppercase"
          required
        />
    ...
    <div
      v-if="error"
      class="mt-4 rounded border border-red-500/20 bg-red-500/10 p-3 text-sm text-red-500"
    >
      {{ error }}
    </div>
